...
def traiter_requete_ussd(requete_str):
    """
    Traite la requête reçue de l'ESP32, valide le code PIN de l'abonné ID 2
    ...
    # Simulation : L'interface est maintenant liée au deuxième abonné de test du fichier JSON (ID 2 / Index 1)
    if len(abonnes) > 1:
        expediteur = abonnes[1]
    else:
        # Sécurité pour éviter un crash d'index hors limites si le JSON a été tronqué ou vidé
        expediteur = abonnes[0]
        print("[!] Attention : L'abonné ID 2 n'a pas été trouvé dans la base. Repli temporaire sur l'abonné ID 1.")
    
    pin_attendu = expediteur.get("code_pin", "")
    solde_expediteur = expediteur.get("solde", 0.0)
    
    # Récupération détaillée de l'identité de l'expéditeur (ID 2)
